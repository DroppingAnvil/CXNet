 ...
 *
 * fromBrowser: true when the request arrived via the loopback HTTP bridge (Chrome extension).
 * false (default) when called in-process from CXNexus. CXAppServer enforces browserEnabled()
 * against this flag. This is a client-managed policy flag with no cryptographic enforcement.
 * A client modified against spec can bypass it; that is considered out of scope.
 */
public class CXAppRequest {
    ...
    public CXAppRequest(String appID, String op, String target, String[] args, boolean fromBrowser) {
        this.appID       = appID;
        this.op          = op;
        this.target      = target;
        this.args        = args;
